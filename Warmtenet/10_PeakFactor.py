...
if optimize == True:
    ...
    PKhuis = pd.DataFrame({
        '9:00': buildings['area'] / random.sample(np.arange(400, 500, 0.1).tolist(), int(len(buildings))),
        '17:00': buildings['area'] / random.sample(np.arange(300, 1000, 0.1).tolist(), int(len(buildings)))})
    ...
    m.PKhuis = Param( m.h, m.t, within=NonNegativeReals, initialize = PKhuis.stack().to_dict())
    ...
    m.Q = Var(m.x, m.x, m.t, domain=NonNegativeReals)
    ...
    m.Q_ht = Var(m.h, m.t, domain=NonNegativeReals)
    ...
    def Discharge_Con(m, x, x2, t):  # 3
        return m.A[x, x2] * m.U >= m.Q[x, x2, t]


    def Power_Con(m, h, t):  # 4
        return m.Q_ht[h,t] * (m.ro * m.sw * (m.T_in - m.T_out[h])) == \
               m.Conn[h] * m.PKhuis[h,t]



    def Q_mass_balance(m, x, t):
        if (x > m.H) and (x < m.X):
            return sum(m.Q[x, x2, t] for x2 in m.x) == sum(m.Q[x2, x, t] for x2 in m.x)
        elif x == m.X:
            return sum(m.Q_ht[h, t] for h in m.h) == sum(m.Q[x2, x, t] for x2 in m.x)
        else:
            return Constraint.Skip


    def Q_initial(m, h, t):
        return  sum(m.Q[h, x2, t] for x2 in m.x) ==  m.Q_ht[h,t]


    def Q_source_Con(m, t):
        return sum(m.Q_ht[h,t] for h in m.h) * (m.T_in - m.T_source_in) <= \
    ...
    def pipes_construction_Con(m, x, x2, t):
        return m.Q[x, x2, t] <= m.Q_poss[x, x2]
    ...
    m.DischargeConstraint = Constraint(m.x, m.x, m.t, rule=Discharge_Con)  # 3
    m.PowerConstraint = Constraint(m.h, m.t, rule=Power_Con)  # 4

    # pipes
    m.Q_sourceConstraint = Constraint(m.t, rule=Q_source_Con)
    m.Q_massBalanceConstraint = Constraint(m.x, m.t, rule=Q_mass_balance)
    m.onedirectionConstraint = Constraint(m.x, m.x, m.t, rule=one_direction_con)
    m.PipesConstructionConstraint = Constraint(m.x, m.x, m.t, rule=pipes_construction_Con)
    m.QInitialConstraint = Constraint(m.h, m.t, rule=Q_initial)
