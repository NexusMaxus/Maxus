...
m.C_Street = Param(m.b, within=NonNegativeReals)
...
m.Link_built = Var(m.b,m.b,domain=NonNegativeIntegers)
m.Link_Boolean = Var(m.b,m.b,domain=NonNegativeIntegers)
...
def Cost_Streets_Con(m):
    return m.CostStreets == sum(sum((m.Link_built[b,b2]*100)**2/((m.Link_built[b,b2]*100)**2+1) * m.C_Street[b] for b in m.b)for b2 in m.b)

def link_built_boolean_Con(m,b,b2):
    return m.Link_Boolean[b,b2] == (m.Link_built[b,b2]*100)**2/((m.Link_built[b,b2]*100)**2+1)

def discharge_transported_Con(m,b,b2):
    return m.Link_built[b,b2] == m.Link_b[b,b2] * m.Q[b]
...
m.dischargeTransportedConstraint = Constraint(m.b,m.b, rule=discharge_transported_Con)
m.LinkBuiltBooleanConstraint = Constraint(m.b,m.b,rule=link_built_boolean_Con)
