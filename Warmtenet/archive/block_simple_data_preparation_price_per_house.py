...
for pand in points_with_house.loc[:,'pandidentificatie']:
    p2a[pand] = houses_data[houses_data['PandBAGnr'] == pand].index
...
pkhuis = houses_data["GJHT"]/365*3
wvhuis = houses_data["GJHT"]
p_h = houses_data["WprijsHT"]
...
# m.P_grid = Var(domain=pmo.NonNegativeReals, initialize=5, bounds=(0, 15))
...
def junction(b, x):
    ...
    b.street_open_con = Constraint(b.i, rule=steet_open_rule)
...
def house(b, h):
    ...
    b.revenue = Var(domain=pmo.NonNegativeReals, initialize=0)

    def Reven_Con(b):
        return b.revenue == b.P_h * b.WV * b.Conn
    ...
    b.RevenueConstraint = Constraint(rule=Reven_Con)
...
def obj_rule(m):
    return sum( m.d[h].revenue * m.d[h].Conn for h in m.h)*10 - sum(m.p[x].street_cost for x in m.x)
...
# results = opt.solve(instance,  solver='ipopt',  strategy='rand_guess_and_bound', stopping_mass=0.8, HCS_tolerance = 0.00001)
                    # solver_args = {'acceptable_tol': 0.0001, 'constr_viol_tol':0.0001, 'acceptable_constr_viol_tol':0.0001})
# options=)
instance.solutions.store_to(results)
# instance.pprint()
instance.display('results_14_block_simple_constraints.txt')

results.write(filename='results_14_block_simple.json', format='json')
