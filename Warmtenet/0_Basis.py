...
m.T_uit = Param(within=NonNegativeReals)
...
m.T_uit_tot = Var(domain=NonNegativeReals)
...
def Power_Con(m,h):  #4
    return m.Q_h[h] * (m.ro * m.sw * (m.T_in - m.T_uit)) ==  sum(m.Type[w,h] * m.PKhuis[w,h] for w in m.w)

def Type_Con(m,h):  #5
    return sum(m.Type[w,h] for w in m.w) == 1

def Temp_Con(m): #10
    return m.T_uit_tot ==  m.T_uit  #sum (m.Q_h[h] * m.T_uit for h in m.h)/sum m.Q_h[h] for h in m.h)

def Q_Con(m,h):  #11
    return m.Q[h] == m.Q_h[h]

def Source_Con(m): #12
    return m.PK_bron == sum(m.Q[b] for b in m.b) * (m.ro * m.sw * (m.T_in - m.T_uit_tot))
...
m.temperatureConstraint = Constraint(rule=Temp_Con)
